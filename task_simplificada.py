...
@dag(
    dag_id='tweets_classifying',
    schedule_interval="@daily",
    start_date=pendulum.datetime(2023, 11, 30),
    catchup=False
)
def tweets_classifying():
  fetch_data_task = fetch_data(WANDB_API_KEY)
  data_exploration_task = data_exploration()
  preprocessing_data_task = preprocessing_data()
  data_check_task = data_check()
  data_segregation_task = data_segregation(run)
  data_train_task = data_train(run)
  wandb_finish_task = wandb_finish(run)

  fetch_data_task >> data_exploration_task >> preprocessing_data_task

  data_check_task

  data_segregation_task >> data_train_task >> wandb_finish_task
